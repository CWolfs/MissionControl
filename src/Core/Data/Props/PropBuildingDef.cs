    ...
    private void OnDeserialized(StreamingContext context) {
      ...
      foreach (JObject destructibleFlimsy in RawDestructibleFlimsyModels.Children<JObject>()) {
        ...
        if (DataManager.Instance.DestructibleDefs.ContainsKey(key)) {
          ...
          Main.Logger.LogError($"[PropBuildingDef.OnDeserialized] PropBuildingDef '{Key}' cannot find PropDestructibleFlimsyDef with key '{key}'. Check your /props/destructibles fodler has a valid definition json for it.");
        }
      }
    }
  }
