...
namespace MissionControl.Config {
    ...
    public bool ShouldEliteLancesBeSelected(Faction faction) {
      ...
      if (useEliteLances && Conditions.Contains("IsEnemy")) {
        useEliteLances = UnityGameInstance.Instance.Game.Simulation.IsFactionEnemy(faction);
      }

      if (useEliteLances && Conditions.Contains("IsAlly")) {
        useEliteLances = UnityGameInstance.Instance.Game.Simulation.IsFactionAlly(faction);
      }
